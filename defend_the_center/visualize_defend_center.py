...
def make_eval_env(render: bool = True, doom_skill: int = 3):

    def _make() -> gym.Env:
        render_mode = "human" if render else None
        env = gym.make("VizdoomDefendCenter-v0", render_mode=render_mode)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Visualize a trained agent on Defend The Center")
    parser.add_argument("--algo", type=str, default="ppo", choices=["ppo", "dqn"], help="Model algorithm")
    parser.add_argument(
        "--model-path",
        type=Path,
        required=True,
        help="Path to the .zip model (e.g. runs/ppo_defend_center/eval/best_model/best_model.zip)",
    )
...
def main() -> None:
    ...
    # Validate that the model expects exactly the same observation (12, 84, 84)
    try:
